...
{
	NSString* query=[NSString stringWithFormat:@"select af, message, thetime, message_history_id, delivered from (select ifnull(actual_from, message_from) as af, message,     timestamp  as thetime, message_history_id, delivered from message_history where account_id=%@ and (message_from='%@' or message_to='%@') order by message_history_id desc limit 20) order by message_history_id asc",accountNo, buddy.escapeForSql, buddy.escapeForSql];
	DDLogVerbose(@"%@", query);
