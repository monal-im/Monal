...
{
    NSString* query=[NSString stringWithFormat:@"select distinct X.*, 0 as 'count' from (select distinct a.buddy_name,state,status,filename, ifnull(b.full_name, a.buddy_name) as full_name, a.account_id from activechats as a left outer  join buddylist as b on a.buddy_name=b.buddy_name and a.account_id=b.account_id ) as X left outer join (select account_id, message_from, message_to,  max(timestamp) as max_time from  message_history group by account_id, message_from,message_to) as Y on X.account_id=Y.account_id AND (X.buddy_name = Y.message_from OR X.buddy_name = Y.message_to) order by Y.max_time desc, X.full_name COLLATE NOCASE asc" ];
    //	DDLogVerbose(query);
     [self executeReader:query withCompletion:^(NSMutableArray *results) {
         if(completion) completion(results);
     }];
