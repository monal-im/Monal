use bytes::Buf;
use rxml::{error::EndOrError, Context, Event, Options, Parse, Parser, WithOptions, XmlVersion};
...
impl MonalXmlStreamParser {
    ...
    pub fn feed(&mut self, chunk: &str) {
        self.pending.extend(chunk.as_bytes());
    }
                ...
                match event {
                    ...
                    Some(Event::StartElement(_statistics, (ns, ncname), attributes)) => {
                        ...
                        let attrs = attributes
                            .iter()
                            .map(|((_ns, name), value)| (name.to_string(), value.to_string()))
                            .collect();
