...
func sendSMSRequest(to number:String) -> Promise<(data: Data, response: URLResponse)> {
    let device = HelperTools.hardwareString()
    ...
    rq.addValue(device, forHTTPHeaderField: "Device")
    
    //add hmac based authentication
    rq.addValue(HelperTools.generateQuicksyAuthorization(
        withNumber: number,
        installationId: rq.value(forHTTPHeaderField: "Installation-Id")!,
        userAgent: rq.value(forHTTPHeaderField: "User-Agent")!,
        andDevice: device
